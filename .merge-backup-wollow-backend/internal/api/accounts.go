...
func (s *Server) handleListAccounts(w http.ResponseWriter, r *http.Request) {
	rows, err := s.DB.Query(`SELECT id, label, provider_type, imap_host, imap_port, smtp_host, smtp_port, username, use_tls, created_at FROM accounts ORDER BY id`)
	if err != nil {
...
func (s *Server) handleCreateAccount(w http.ResponseWriter, r *http.Request) {
	...
	if req.Label == "" || req.ImapHost == "" || req.ImapPort == 0 || req.Username == "" || req.Password == "" {
		writeError(w, http.StatusBadRequest, "label, imapHost, imapPort, username, and password are required")
		return
	}
	...
	if err != nil {
		writeError(w, http.StatusBadRequest, "could not connect to IMAP server: "+err.Error())
		return
	}
	...
	result, err := s.DB.Exec(
		`INSERT INTO accounts (label, provider_type, imap_host, imap_port, smtp_host, smtp_port, username, encrypted_password, use_tls) VALUES (?, 'imap', ?, ?, ?, ?, ?, ?, ?)`,
...
func (s *Server) loadAccountCredentials(id int64) (mail.AccountCredentials, error) {
	...
	var port, useTLS int
	err := s.DB.QueryRow(
		`SELECT imap_host, imap_port, username, encrypted_password, use_tls FROM accounts WHERE id = ?`, id,
