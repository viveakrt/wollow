...
export function Transactions() {
  ...
  function reload() {
    ...
    return api.transactions
      .list({ search, accountId, categoryId, type, limit: 200 })
      .then((rows) => {
  ...
  async function handleBulkDelete() {
    ...
    setBulkBusy(true)
    try {
      await api.transactions.bulkDelete([...selected])
      setSelected(new Set())
      await reload()
    } finally {
      setBulkBusy(false)
  ...
  async function handleBulkCategorize(catId: number | null) {
    if (selected.size === 0) return
    setBulkBusy(true)
    setShowCategoryMenu(false)
    try {
      await api.transactions.bulkCategorize([...selected], catId)
      setSelected(new Set())
      await reload()
    } finally {
      setBulkBusy(false)
  ...
  async function handleLinkTransfer() {
    ...
    setBulkBusy(true)
    try {
      await api.transactions.linkTransfer(selectedTxns[0].id, selectedTxns[1].id)
      setSelected(new Set())
      await reload()
    } finally {
      setBulkBusy(false)
            ...
            {showCategoryMenu && (
              <div className="absolute right-0 top-full mt-1 w-56 max-h-64 overflow-y-auto bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-lg shadow-xl z-10 py-1">
                <button
                  ...
                  className="w-full text-left px-3 py-2 text-sm text-[var(--color-text-muted)] hover:bg-white/5"
                >
                  Uncategorized
                </button>
                {categories.map((c) => (
                ...
                transactions.map((t) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      {t.type === 'transfer' ? (
                        <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs bg-white/5 text-[var(--color-text-muted)]">
                          <Shuffle size={11} />
                          Transfer
                        </span>
                      ) : t.categoryName ? (
