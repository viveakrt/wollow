...
export function Transfers() {
  ...
  function reload() {
    return api.transferSuggestions.list().then(setSuggestions).finally(() => setLoading(false))
  }
  ...
  async function handleScan() {
    setScanning(true)
    try {
      await api.transferSuggestions.scan()
      await reload()
    } finally {
      setScanning(false)
    }
  ...
  async function handleConfirm(id: number) {
    setBusyId(id)
    try {
      await api.transferSuggestions.confirm(id)
      await reload()
    } finally {
      setBusyId(null)
    }
  ...
  async function handleDismiss(id: number) {
    setBusyId(id)
    try {
      await api.transferSuggestions.dismiss(id)
      await reload()
    } finally {
      setBusyId(null)
    }
    ...
    <div className="p-8 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          onClick={handleScan}
          disabled={scanning}
          className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[var(--color-accent)] text-white text-sm font-medium hover:bg-violet-500 disabled:opacity-50 transition-colors whitespace-nowrap"
        >
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-500/15 text-emerald-400 text-xs font-medium hover:bg-emerald-500/25 disabled:opacity-50"
                  >
                    <Check size={13} />
                    Confirm
                  </button>
