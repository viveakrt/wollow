...
func (s *Server) handleDashboardSummary(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.DB.QueryRow(`SELECT COALESCE(SUM(current_balance),0) FROM finance_accounts`).Scan(&summary.NetWorth); err != nil {
	...
	rows, err := s.DB.Query(`
		SELECT id, name, account_type, bank, current_balance FROM finance_accounts ORDER BY id`)
	if err != nil {
		httpx.WriteError(w, 500, err.Error())
		return
	}
